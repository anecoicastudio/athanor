...
-- Ranking: text relevance (similarity) + id tie-break only — never aura/circle/entitlement (rule #1).
...
-- profiles: handle + bio + tag arrays flattened to text (identity_tags, seeking)
...
create index profiles_search_trgm on public.profiles using gin (
  (public.f_unaccent(coalesce(handle,'') || ' ' || coalesce(bio,'') || ' ' ||
   array_to_string(identity_tags,' ') || ' ' || array_to_string(seeking,' '))) extensions.gin_trgm_ops
);
...
create function public.search_all(
  ...
  cursor_rank  real    default null,       -- keyset cursor (similarity rank)
  ...
  with hits as (
    ...
    select 'person'::text as entity_type, p.id,
           ...
           extensions.similarity(
             public.f_unaccent(coalesce(p.handle,'') || ' ' || coalesce(p.bio,'') || ' ' ||
               array_to_string(p.identity_tags,' ') || ' ' || array_to_string(p.seeking,' ')), needle) as rank
    from public.profiles p
    where scope in ('all','people')
      and public.f_unaccent(coalesce(p.handle,'') || ' ' || coalesce(p.bio,'') || ' ' ||
            array_to_string(p.identity_tags,' ') || ' ' || array_to_string(p.seeking,' ')) % needle
