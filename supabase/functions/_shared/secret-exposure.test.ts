...
//
// SPEC-FIRST. docs/PRD.md §8 — "Checkout/Billing session (created by edge fn, **never
// client-side keys**)". docs/PRD.md §9 — "Stripe keys server-side only". CLAUDE.md rule 6 and
// rule 8 say the same, and the stripe-best-practices skill (references/security.md, sections
// «API keys» and «Mobile and client-side integrations», plugin 0.9.2, read 2026-09-26) makes
// it Stripe canon: keys never in source, never in client-side code.
//
...
Deno.test('no LIVE Stripe key exists anywhere on this checkout', () => {
  // security.md «API keys» — a string matching /[sr]k_live_.*/ IS a live key, wherever it sits.
  // Local .env files are included on purpose: this is the incident-response check, not a style
  // check.
...
Deno.test('no Stripe key or webhook secret is committed to source', () => {
  ...
  // source file (security.md «API keys»). `.env*` files are exempt because they ARE
...
Deno.test('no Stripe server secret is reachable from the client bundle', () => {
  // docs/PRD.md §8 — the session is minted by an edge fn, "never client-side keys".
