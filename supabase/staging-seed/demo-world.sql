...
-- All twenty-four, English. identity_tags / seeking / skills / profession are the curated keys
-- of packages/core/src/onboarding/{tags,skills,professions}.ts — the database does not constrain
-- them, and the app renders an off-list value as the stored text, untranslated (#883), so a value
-- that is not a key is a seed bug. identity_verified and founding_member are left as
-- seed-staging.sql set them.
...
from (values
  ('sole_designer',  'Designer. Small studio, projects that leave the world a little clearer.',        'design',                  'Milano',  'Design only what I believe in, and live from it.',                 array['creativo','freelance'],       array['collaborazioni','connessioni'], array['branding','ui-ux','illustrazione']),
  ('luna_dev',       'Developer. Building things that help people sleep better.',                        'sviluppo',                'Milano',  'Software that gives people their evenings back.',                  array['freelance','creativo'],       array['collaborazioni','crescita'],    array['sviluppo-mobile','sviluppo-web','dati']),
  ('marta_ceramica', 'Ceramicist. Wheel, glazes, and a kiln that deserves to be fired more often.',     'artigianato',              'Milano',  'A workshop where anyone can learn to make something that lasts.',  array['artista','freelance'],        array['business','connessioni'],       array['fotografia','social-media']),
  ('gio_musica',     'Producer. I record in a basement with great acoustics and terrible wifi.',         'musica',                  'Milano',  'Records made where the musicians actually live.',                  array['artista','creativo'],         array['collaborazioni','eventi'],      array['produzione-musicale','sound-design']),
  ('ele_yoga',       'Yoga teacher. I take the practice where it usually does not reach.',               'benessere',               'Milano',  'Movement for the people everyone forgot to invite.',               array['coach','freelance'],          array['connessioni','eventi'],         array['coaching','facilitazione']),
  ('tino_chef',      'Chef. Looking for producers who make things the way they used to.',                'food',                    'Milano',  'Cook what the valley gives, and pay the people who grow it.',      array['imprenditore','creativo'],    array['business','collaborazioni'],    array['cucina','vendite']),
  ('vera_erbe',      'Herbalist. I pick, I dry, I listen.',                                              'benessere',               'Como',    'Keep the knowledge of the paths alive, one plant at a time.',      array['artista','freelance'],        array['crescita','connessioni'],       array['fotografia','storytelling']),
  ('rocco_film',     'Filmmaker. Short documentaries about crafts that are disappearing.',               'foto-video',              'Genova',  'Film the hands before the hands are gone.',                        array['artista','creativo'],         array['collaborazioni','crescita'],    array['videomaking','montaggio','storytelling']),
  ('sara_startup',   'Founder. Second time around, slower on purpose.',                                  'business',                 'Milano',  'A company that is still good to work at when it is forty people.', array['imprenditore','investitore'], array['mentorship','business'],        array['fundraising','project-management','vendite']),
  ('dario_legno',    'Carpenter. Reclaimed wood, visible joints, no screws.',                            'artigianato',               'Bergamo', 'Teach the trade to the ones nobody else will hire.',               array['artista','mentor'],           array['eventi','collaborazioni'],      array['falegnameria','facilitazione']),
  ('nina_poeta',     'I write. Mostly at night, mostly by hand.',                                        'scrittura',                    'Milano',  'Poems that get read aloud, not scrolled past.',                    array['creativo','artista'],         array['crescita','connessioni'],       array['copywriting','storytelling','traduzione']),
  ('bea_foto',       'Photographer. Long portraits, film when I can.',                                   'foto-video',              'Milano',  'A portrait of every workshop in the old town before it closes.',   array['freelance','artista'],        array['collaborazioni','eventi'],      array['fotografia','montaggio']),
  ('noah_climbs',    'Climbing coach. I teach kids to fall well before I teach them to go up.',          'benessere',               'Milano',  'A wall for every kid who cannot afford the gym.',                  array['coach','mentor'],             array['connessioni','eventi'],         array['coaching','facilitazione']),
  ('amara_textiles', 'Textile designer. Weaving on a loom older than me.',                               'artigianato',             'Torino',  'Fabric made together by people who arrived and people who stayed.', array['artista','imprenditore'],    array['collaborazioni','business'],    array['illustrazione','branding']),
  ('leo_bikes',      'Bike mechanic. Every bike deserves a second life.',                                'artigianato',             'Milano',  'A repair café in every neighbourhood of the city.',                array['imprenditore','mentor'],      array['collaborazioni','eventi'],      array['facilitazione','project-management']),
  ('hana_garden',    'Urban gardener. Tomatoes on rooftops, compost in the stairwell.',                  'food',                    'Milano',  'Rooftops that feed the building below them.',                      array['creativo','freelance'],       array['connessioni','collaborazioni'], array['facilitazione','social-media']),
  ('theo_code',      'Developer. I write small tools for people who fix things.',                        'sviluppo',                'Milano',  'Software for the people who keep things working.',                array['freelance','creativo'],       array['collaborazioni','crescita'],    array['sviluppo-web','dati','no-code']),
  ('ines_dance',     'Dancer. I choreograph for people who think they cannot dance.',                    'arte',                    'Milano',  'A stage big enough for a whole neighbourhood.',                    array['artista','coach'],            array['eventi','connessioni'],         array['facilitazione','storytelling']),
  ('omar_bread',     'Baker. Night shifts, long fermentation, short conversations.',                     'food',                    'Bologna', 'A bakery that trains young people and pays them properly.',        array['imprenditore','mentor'],      array['business','mentorship'],        array['cucina','vendite']),
  ('clara_books',    'Bookbinder and bookseller. Paper, thread and patience.',                           'artigianato',              'Milano',  'Books that outlive the phones we read them on.',                   array['artista','imprenditore'],     array['collaborazioni','eventi'],      array['copywriting','illustrazione']),
  ('jonas_sound',    'Field recordist. I collect the sounds a city forgets it makes.',                   'musica',                  'Milano',  'An archive of the city you can listen to.',                        array['artista','freelance'],        array['collaborazioni','crescita'],    array['sound-design','produzione-musicale']),
  ('priya_care',     'Nurse, twelve years in emergency. Now building something slower.',                 'benessere',                   'Milano',  'Care for the people who fall between the cracks.',                 array['coach','imprenditore'],       array['mentorship','connessioni'],     array['coaching','project-management']),
  ('sam_mentor',     'Retired engineer. Forty years of bridges; now I help people build their first thing.', 'educazione',               'Milano', 'Fifty first-time founders before I turn seventy.',             array['mentor','investitore'],       array['mentorship','connessioni'],     array['project-management','contabilita','fundraising']),
  ('zoe_murals',     'Muralist. Walls are the only gallery everyone can enter.',                         'arte',                    'Milano',  'Paint the city with the faces of the people who live in it.',      array['artista','creativo'],         array['collaborazioni','eventi'],      array['illustrazione','branding'])
