...
update public.profiles pr set
  ...
  -- #694: a deterministic birth date per handle (1970 + up to 35 years, ages 21–56 in 2026),
  -- so a re-seed never flips a member's sign and every persona clears the 18+ guard (#778: the
  -- youngest date the formula can reach is 2004-12-31, and a persona only ever ages). The sign
  -- itself is generated by the column. Hashing twelve handles does not promise all twelve
  -- signs render — if QA needs a specific sign, pin the date, don't reseed for luck.
  birth_date = date '1970-01-01' + (('x' || left(md5('dob:' || p.handle), 7))::bit(28)::int % 12784),
...
union all select 'profiles with birth_date (#694)', count(*) from public.profiles where birth_date is not null
