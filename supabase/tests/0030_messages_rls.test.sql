...
select public.create_conversation_pair(
  '11111111-1111-1111-1111-111111111111','22222222-2222-2222-2222-222222222222','momento') as conv \gset
reset role;
...
select results_eq(
  $$ select count(*)::int from public.messages where conversation_id = (select :'conv')::uuid
...
select lives_ok(
  $$ insert into public.messages (conversation_id, sender_id, kind, body)
     values ((select :'conv')::uuid,'11111111-1111-1111-1111-111111111111','user','Ciao!') $$,
  'participant posts a user message');

-- client cannot insert kind='system' (unforgeable banner) → 42501
select throws_ok(
  $$ insert into public.messages (conversation_id, kind, body)
     values ((select :'conv')::uuid,'system','forged banner') $$,
...
select throws_ok(
  $$ insert into public.messages (conversation_id, kind, prompt_key)
     values ((select :'conv')::uuid,'prompt','chat.prompt.who') $$,
  '42501', null, 'client cannot forge a prompt ice-breaker');

-- client cannot spoof another sender → 42501
select throws_ok(
  $$ insert into public.messages (conversation_id, sender_id, kind, body)
     values ((select :'conv')::uuid,'22222222-2222-2222-2222-222222222222','user','spoofed') $$,
  '42501', null, 'client cannot send as another user');

-- bump trigger set last_message_at + preview from the user message
select results_eq(
  $$ select last_message_preview from public.conversations where id = (select :'conv')::uuid $$,
...
select throws_ok(
  $$ insert into public.messages (conversation_id, sender_id, kind, body)
     values ((select :'conv')::uuid,'33333333-3333-3333-3333-333333333333','user','intruder') $$,
...
select throws_ok(
  $$ insert into public.messages (conversation_id, sender_id, kind, body, prompt_key)
     values ((select :'conv')::uuid,'11111111-1111-1111-1111-111111111111','user','hi','chat.prompt.who') $$,
