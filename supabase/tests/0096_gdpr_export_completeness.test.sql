...
insert into gdpr_excluded values
  ...
  ('report_alert_sends', 'athanor: dispatch dedupe markers for the moderation queue alert (#602). Records that a report was announced to a WATCHER, not anything the member did — the reported member is not even referenced, only the report id and the admin recipient. Reaped once the marker is 30 days old AND its report is no longer unresolved (both, so a live report never loses its memory), dropped with the profile by FK'),
