...
vi.mock('@athanor/api', () => ({
  updateOnboardingProfile: async (_client: unknown, _userId: string, answers: unknown) => {
...
describe('flushOnboardingDraft', () => {
  it('flushes a complete draft, birth_date included, and clears it', async () => {
    await expect(flushOnboardingDraft('u1')).resolves.toBe('flushed');
    expect(api.updates).toHaveLength(1);
    expect(api.updates[0]).toMatchObject({ birth_date: '1990-08-10' });
    expect(draftState.cleared).toBe(true);
  });

  // #782: the handle is chosen on its own screen once the account exists. The flush used to
  // derive it from the email — `lucia@example.com` became @lucia — and that is the leak.
  it('writes no handle — nothing is derived from the email any more', async () => {
    await expect(flushOnboardingDraft('u1')).resolves.toBe('flushed');
    expect(api.updates[0]).not.toHaveProperty('handle');
  });
  ...
  it('reports nodraft when the draft has no birth date — the funnel gate, not the flush, asks for it', async () => {
    ...
    await expect(flushOnboardingDraft('u1')).resolves.toBe('nodraft');
  ...
  it('drops a draft the schema refuses (an impossible day) rather than looping on it', async () => {
    ...
    await expect(flushOnboardingDraft('u1')).resolves.toBe('error');
    expect(api.updates).toHaveLength(0);
    expect(draftState.cleared).toBe(true);
  });
