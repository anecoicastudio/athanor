 ...
 * the failure the `fetchWeekRecap` docblock in `lib/week-recap.ts` describes for `auraKeys.recap`,
 * which is guarded by convention there and by construction here.
 *
 ...
 * you have already favored (its definition in `20260615081559_favor_offers.sql`) — so a non-empty
 * first page IS "there is something you could do", with no client-side filtering.
 */
