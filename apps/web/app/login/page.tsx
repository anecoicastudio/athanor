...
function LoginForm() {
  ...
  if (state === 'sent') {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-background px-6">
        <h1 className="text-3xl text-foreground">{t('auth.sent.title', 'it')}</h1>
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-8 bg-background px-6">
      <h1 className="max-w-xl text-center text-5xl text-foreground">{t('auth.welcome.title', 'it')}</h1>
      ...
      <form onSubmit={sendLink} className="flex w-full max-w-sm flex-col gap-4">
        <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          ...
          <input
            ...
            className="mt-2 w-full rounded-full border border-border bg-card px-5 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-foreground"
          />
        </label>
        {state === 'error' ? (
        ...
        <button
          ...
          className="h-12 rounded-full bg-foreground font-semibold tracking-widest text-background disabled:opacity-60"
        >
          {t('auth.email.cta', 'it')}
        </button>
      </form>
