...
- Paid tickets: Stripe Checkout as a **destination charge** (#104, ruling 2026-09-06). The fee is **absorbed**: the buyer pays the displayed price unchanged, Athanor retains `events.fee_pct` (default 10%) as the `application_fee_amount`, and the organiser's connected account receives the rest — Stripe's processing is paid by the platform out of the fee, not deducted from the organiser. Stripe splits at payment time; there is no manual settlement. Publishing a paid event therefore requires a connected account with `payouts_enabled`, beside the identity gate. Free events: 1-tap RSVP.
