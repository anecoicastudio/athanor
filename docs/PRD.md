...
- Public web version at `www.athanor.world/@handle` (SSR, SEO, OG image with dream quote) — only fields marked public. (The domain is `https://www.athanor.world` (#471, 2026-09-03; `athanor.app` was on the aftermarket): `apps/web/lib/site.ts:10` holds the canonical origin, and `apps/web/lib/site.test.ts:32-41` asserts it equals the host `apps/native/app.json` associates with, so the two cannot drift. Deep links only work on that host — AASA, `assetlinks` and `app.json` move together with it or not at all.)
