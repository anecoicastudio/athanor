...
- A row that lands on `failed` or `partial` leaves the account **banned and intact**, and the ban
  stays sticky until the row is `done` or deleted. That is the right state for someone who asked
  to be erased; the re-queue below finishes it, and the `update … set status = 'requested'` fires
  the same trigger a fresh request does.
