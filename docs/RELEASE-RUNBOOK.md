...
**Prerequisite met 2026-09-24:** production's `remote_config` holds `apple_signin_enabled = {"enabled": true}` (set 12:49 UTC) and the Apple provider is on (`external_apple_enabled`, read through the Management API), so the sign-in sentence below is true (#79). Google ships unconditionally: if the flag or the provider is ever switched off, take the Apple half of that sentence out before pasting, or 4.8 is the rejection. Re-check before every paste the two auth facts the sign-up sentence rests on, as queried on 2026-09-24: `mailer_autoconfirm = true`, so a new account needs no confirmation email, and the per-IP limit on sign-ups and sign-ins is at its default of 30 requests per 5 minutes, which one reviewer's sign-up never reaches. If confirmation mails are turned on, drop «no confirmation email is needed».

> Athanor is an Italian community app. The demo account is set to English; Settings → Language switches every screen to Italian. The app follows the language chosen there, not the device's.
>
...
Keep the notes to what the reviewer will see. Do not mention the fund cycle, Prime Stelle or any other surface behind a flag that is OFF; paid tickets and the fund are named only to say they are off.
...
- A row that is not `done` — `requested`, `processing`, `partial`, `failed` — means the account is
  **banned and intact**, and the ban stays sticky until the row is `done` or deleted. That is the
  ...
  a bare NULL. Then run the reconcile below in the same release window.
