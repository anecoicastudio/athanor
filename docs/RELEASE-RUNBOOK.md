...
| `post-media-reaper` edge-function deploy                                                                                                                                                                                                                                                                                                  | ⏳ STAGING ONLY — deployed to staging 2026-08-28 (#589) and hand-invoked there once, which reaped 2 orphaned objects (15 MB). Production has neither the function nor the Vault pair, and the `reap-post-media-bytes` cron job is a quiet no-op on both projects until `app.settings.post_media_reaper_url` + `_key` exist. See the deploy rider in §5.                                                                                                                                                                                                                                                                     |
...
-- 2. Re-queue them. Do this in ONE statement, and read the count it reports back.
...
-- 4. A few seconds later: every row should read 'done', with profile_id NULL.
