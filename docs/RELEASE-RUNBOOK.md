...
> **Handle rename purge deploy config (2026-09-25, #800):** a rename used to leave `/@old` and its OG card in the KV incremental cache, still serving the member's photo, name and dream quote — and, once the handle was reclaimed, serving them at somebody else's address. `20260925124457` adds `profiles_handle_rename_purge`, an AFTER UPDATE OF handle trigger that posts `{"handle":"<old>"}` through `athanor.enqueue_handle_rename_purge()` to the NEW `handle-rename-purge` edge function, which runs the same sweep as erasure (`_shared/kv-purge.ts`, every build prefix) for `/@old` and `/@old/opengraph-image`. It needs the `CF_KV_*` trio erasure already uses (§7.2) and nothing else in function env. Order: **push the migration, deploy the function, then create the Vault pair**: `select vault.create_secret('https://<ref>.supabase.co/functions/v1/handle-rename-purge', 'app.settings.handle_rename_purge_url')` and `select vault.create_secret('sb_secret_…', 'app.settings.handle_rename_purge_key')` — NOT `alter database … set` (42501 hosted); same pattern as the reapers above. **Until both secrets exist every rename is a silent no-op** — no pg*net row, no error, and `/@old` stays cached — which is the #468/#492 shape, so this rider is not optional on either project. The trigger is fail-open by design (a cache purge must never fail the rename). **Verify** by renaming a throwaway handle and reading the response a few seconds later: `select id, created, status_code, timed_out, content from net._http_response where content like '%"scanned"%' or timed_out order by id desc limit 3` → expect `200 {"deleted":N,"scanned":M}`. `503 {"configured":false}` means the `CF_KV*\*`trio is missing;`502`means the sweep did not finish (a Cloudflare list or delete failed, or the 100-page cap was hit) and the old entries may still be there — nothing retries it, so run §7.4's manual sweep for that handle.`net.\_http_response` is purged after pg_net's ttl, so read it the same day.
