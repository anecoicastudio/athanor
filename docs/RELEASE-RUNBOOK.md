...
- **A `partial` row whose account no longer exists is already reconciled by the schema.**
  ...
  gone. Re-queueing such a row makes the job claim a request with no subject; the RPCs match
  nothing and it lands on `done` truthfully, which is the outcome you want, but it is worth
  knowing that `done` there means «nothing left to do», not «work performed».
