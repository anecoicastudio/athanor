...
A crash from the store is a stack of Hermes bytecode offsets until the source map and the debug
files for that exact build are in Sentry. Nothing recovers them afterwards: the upload happens
during the build or not at all, so a production build that skipped it ships crashes nobody can
read.
