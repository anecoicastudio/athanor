...
| Circle (§4.12)           | `circle`, `payments`, `search`, `search-filters`. Hidden on iOS for IAP reasons                     | `circle_memberships` as a cache of Stripe Billing; checkout and portal functions; an entitlement hook gates the features                                                |
