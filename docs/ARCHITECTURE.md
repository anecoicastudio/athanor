...
| GDPR                     | `data-export`, `delete-account`                                                                                          | `consent` + job tables; nightly export (own media incl.) lives 7 days; erasure revokes sessions, deletes content, pseudonymises legally-retained money rows             |
...
| Money tables    | `stripe-webhook` — signature-verified, deduped (two carve-outs: `create-payout-onboarding` inserts the initial `payout_accounts` pointer row; and the GDPR lifecycle — `gdpr_erase_fund_footprint` / `gdpr_erase_payment_footprint`, called by `erasure-job`, tombstone or null the identity, clear the ticket's `qr_token`, stamp `erased_at` and recompute the edition aggregate, keeping every money column and Stripe id on the retained row; since #725 the payment reach also **redacts the `stripe_webhook_events` payloads that name the erased member** — the whole Stripe event is stored there, so the email, name and address the rest of the cascade removed sat on in the ledger — and the `stripe_webhook_events_redact_erased` BEFORE INSERT trigger redacts the deliveries that arrive _after_ the cascade, starting with the cancellation webhook the cascade itself provokes; the dedupe columns (`event_id`, `type`, `received_at`, `processed_at`, `claimed_at`) are untouched by both; ten years later the pure-SQL `gdpr-retention-reap` cron job deletes that row outright and carries its cents into `fund_editions.reaped_cents` so no historic total shrinks. None of them writes a Stripe state transition; that stays the webhook's. `erasure-job` itself cancels the Circle subscription at Stripe, and since #763 clears `metadata.profile_id` off the member's Stripe Customers before the pseudonymisation — a metadata write, so the checkout's tag lookup cannot hand a withdrawn erasure its old Customer) |
...
Stripe is the source of truth; our money tables are a **cache of its webhooks**, written by `stripe-webhook` under the service role (see the two carve-outs in §6 — the GDPR erasure removes who paid while keeping every money column and Stripe id on the retained row, redacts the webhook ledger's stored payloads the same way, clears the member's tag off their Stripe Customers, and the retention reaper deletes that row ten years on, carrying the cents into `fund_editions.reaped_cents`). Verify the signature, then dedupe, then work. And never fulfil on `checkout.session.completed` without checking `session.payment_status` — delayed methods (SEPA) settle days later, and a naive handler ships the goods before the money exists. Stripe keys are server-side only; the app never sees one.
