...
export const milestoneSchema = z.object({
  ...
  body: z
    .string()
    .max(200)
    .refine((value) => value.trim().length > 0, 'milestone body must not be blank'),
...
const milestoneBodySchema = z.string().trim().min(1, 'milestone body must not be blank').max(200);
