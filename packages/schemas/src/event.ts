 ...
 * column is written from lat/long by create_event or a direct INSERT — snapped to a 0.025°
 * grid by the `events_snap_geo` trigger either way (#781) — and never selected by the client;
 * the app reads it only as the distance `events_nearby()` returns (no map). `feePct`/`priceCents`/
